...
const Institutos = () => {
	...
	useEffect(() => {
		const fetchInstitutos = async () => {
			try {
				...
				const institutosData = universidadesData.filter((universidad) => universidad.tipoEscuela === 'Instituto')
	...
	const filteredInstitutos = useMemo(() => {
		...
		return institutos.filter((instituto) => instituto.nombre.toLowerCase().includes(searchValue.toLowerCase()))
				...
				<div className="container-cardU">
					{filteredInstitutos.map((instituto, idU) => (
						<div className="cardU" key={idU}>
							...
							{instituto.enlace && <NextPage to={instituto.enlace} value="Visitar" className="visitar-universidad" />}
						</div>
					))}
				</div>
			</div>
