...
const Universidades = () => {
	...
	const filteredUniversidades = useMemo(() => {
		...
		return universidades.filter(
			(universidad) => universidad.nombre.toLowerCase().includes(searchValue.toLowerCase()) && universidad.tipoEscuela !== 'institutos'
		)
	...
	return (
		<>
			...
			<div className="contenedorU">
				<div className="encabezadoU">{/* Puedes agregar contenido adicional para el encabezado aquí */}</div>
				<div className="universidades">
					<h2>¿Dónde puedo estudiar?</h2>
					<p>
						A continuación te presentamos información de universidades (públicas y privadas) de la región metropolitana de Cochabamba:
					</p>
				</div>
								...
								.map((universidad, idU) => (
									<div className="cardU" key={idU}>
										...
										<div className="contenido-cardU">
											...
											{universidad.enlace && (
												<NextPage to={universidad.enlace} value="Visitar" className="visitar-universidad" />
											)}
									</div>
								))}
						</div>
